...
/* var uploadregsensorbuff = [
	...
	[
		...
		{"POS":"E002","CMD":"LED","STU":"1E0100","Type":"DOSESTU","typecmd":"C71","typereg":"1E"},
...
function  opf403_regdev_loadscan(regpos){ //poslist ,direct buffer LOAD save to buffer load sensor value save to buffer  
	...
	if(regpos.length > 0){
		...
		console.log("===========================");
		//console.log(JSON.stringify(cmdcode));
		for(rr in regpos){
			...
			ttypecmd = regpos[rr].typecmd.substr(1,2)//[0][1] 2 byte
	...
	apipamcheck(res,cmd,uuid,pos,group,cstu,()=>{		
		...
		if(group==0){
			...
			if(cregadd == "20"){ //F5 IPaddr 06 00 01 20 00 00 00
				...
				//                0  1  2  3  4  5  6  7
				//"s71cmdset" :  "f5 01 05 71 04 18 03 71",
				ttbuf = Buffer.from(cmdcode.rs485v060.s71cmdset,'hex'); //"[0][1:add][2:len][3][4:cmd][5:REG][6,7:stu][10]"f5 00 06 00 02 20 12 34 12 34 20"
				cmdindex = pdbuffer.pdjobj.subcmd[cmd]			
				//ttbuf[4]= cmdindex;
